# 튜플tuple
# 파이썬의 리스트와 유사한 자료구조 - 순서기억, 중복저장
# 선언방법은 ()안에 값들을 정의하고 사용
# 단, 튜플의 요소는 추가/수정이나 삭제 불가 (생성은 가능)
# 주로, 읽기전용 데이터를 다룰때 사용 -상수값 저장 ( 속도가 빠름 )

# 튜플 선언
nums = (1, 5, 10, 15, 20)
print(nums)

# 튜플의 요소 읽기 : 객체명[인덱스]
print(nums[0], nums[3])

# 튜플 요소 수정 : 객체명[인덱스] = 수정값
# nums[0] = 999 # error발생

# 튜플 요소 삭제 : 객체명.remove(값), 객체명.pop(), del 객체명[인덱스]
# nums.remove(1) # error발생
# del  nums[1] # error발생

# 만일, 튜플의 요소를 수정/삭제하려면
# 튜플을 리스트로 변환한 후 값을 수정/삭제하고
# 다시 리스트를 튜플로 변환하면 됨 ( 비추 )
# 변환함수 : list(객체명), tuple(객체명)
nums = list(nums) # 튜플 -> 리스트
nums[0] = 999
print(nums)

nums = tuple(nums) # 리스트 -> 튜플
print(nums) 

# ex) 년도를 입력하면 십간과 십이지를 이용해서
# 해당년도의 60갑자(간지) 출력
# 십간 : 갑을병정무기경신임계
# => (10일마다 바뀌는 태양에 부쳐진 이름)
# 십이지 : 자축인묘진사오미신유술해
BASE = 1444
ten = ('갑','을','병','정','무','기','경','신','임','계')
twelve=('자','축','인','묘','진','사','오','미','신','유','술','해')
animal=('쥐','소','호랑이','토끼','용','뱀','말','양','원숭이','닭','개','돼지')
year = int(input('연도 입력 : '))
i1 = ( year - BASE ) % 10
i2 = ( year - BASE ) % 12
#print("%d년은 %s년 (%s의 해) 입니다" % (year, ten[i1] + twelve[i2], animal[i2]))
print(f'{year}년은 {ten[i1]+twelve[i2]}년({animal[i2]}의 해)입니다.')

