...
# 간단한 인사말을 파일에 저장
# open('경로/파일명', '작업모드') py2 방식
f = open('hello.dat', 'w') # 쓰기모드로 파일생성
f.write('Hello, World!!')  # 지정한 파일에 내용쓰기
f.close()                   # 작업종료 위해 파일 닫음

# with open('경로/파일명', '작업모드', '인코딩' ) as 별칭 py3 방식
with open('hello2.dat', 'w', encoding='UTF-8') as f: # 쓰기모드로 파일생성
    f.write('안녕, 세상아!!')    # 지정한 파일에 내용쓰기

# 회원정보를 입력받아 member.dat에 저장
# 저장대상 : 아이디, 비밀번호, 이름, 이메일
# 저장형식 : "아이디/비밀번호/이름/이메일" 형식으로 파일에 저장
# ex) abc123/987xyz/abc123/abc123@987@xtz.co.kr
